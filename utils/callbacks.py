...
class EvalCallback():
    ...
    def get_miou_png(self, image):
        ...
        # The code only supports prediction with RGB images, and all other image types will be converted to RGB.
        #---------------------------------------------------------#
        image       = cvtColor(image)
        orininal_h  = np.array(image).shape[0]
        orininal_w  = np.array(image).shape[1]
        #---------------------------------------------------------#
        # Add padding (gray bars) to the image to achieve distortion-free resizing
        ...
        with torch.no_grad():
            ...
            #--------------------------------------#
            #   Crop out the padding (gray bars) from the image
            #--------------------------------------#
    ...
    def on_epoch_end(self, epoch, model_eval):
        if epoch % self.period == 0 and self.eval_flag:
            ...
            for image_id in tqdm(self.image_ids):
                ...
                #------------------------------#
                #   Obtain the prediction from the txt file
                #------------------------------#
                image       = self.get_miou_png(image)
                image.save(os.path.join(pred_dir, image_id + ".png"))
                        
            print("Calculate miou.")
            _, IoUs, _, _ = compute_mIoU(gt_dir, pred_dir, self.image_ids, self.num_classes, None)  # Execute the function to calculate mIoU (mean Intersection over Union)
